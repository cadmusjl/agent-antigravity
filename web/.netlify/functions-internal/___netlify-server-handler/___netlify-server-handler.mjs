...
} from '\var\task\agent-saas-platform\web/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\agent-saas-platform\web/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\agent-saas-platform\web')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\agent-saas-platform\web',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\agent-saas-platform\web\.netlify\dist\run\handlers\server.js')
